...
export function IncomePanel() {
  const { incomes, incomeByPartner, totalIncome, partnerNames } = useBudget();

  const p1Pct = totalIncome > 0 ? (incomeByPartner.partner1 / totalIncome) * 100 : 50;

  return (
    <div className="rounded-lg bg-card p-6 shadow-warm">
      <h2 className="mb-4 text-lg font-semibold text-foreground">Income</h2>
      ...
      <div className="space-y-2">
        {incomes.map(inc => (
          <div key={inc.id} className="flex items-center justify-between rounded-md bg-surface-alt px-3 py-2 transition-colors hover:bg-muted">
